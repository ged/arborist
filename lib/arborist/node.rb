...
class Arborist::Node
	...
	def make_update_event
		return Arborist::Event.new( 'node.update', self.fetch_values )
	end
	...
	def make_delta_event
		...
		return Arborist::Event.new( 'node.delta', self.update_delta )
	end
	...
	def on_ack( transition )
		...
		self.pending_update_events << Arborist::Event.new( 'node.acked', self.ack.to_h )
	end
